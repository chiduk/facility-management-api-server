...
export const generateAuthTokens = async (user: any): Promise<AccessAndRefreshTokens> => {
  ...
  const accessToken = generateToken(user.uniqueId || user._id, accessTokenExpires, tokenTypes.ACCESS);

  const refreshTokenExpires = moment().add(config.jwt.refreshExpirationDays, 'days');
  const refreshToken = generateToken(user.uniqueId || user._id, refreshTokenExpires, tokenTypes.REFRESH);
  await saveToken(refreshToken, user.uniqueId || user._id, refreshTokenExpires, tokenTypes.REFRESH);
...
export const deleteTokenByRefresh = (refresh: string) =>
  Token.deleteOne({
    token: refresh,
  });
